...
Git.prototype.rm = function (files) {
  ...
  // Если система Windows, используем раздельное удаление
...
/**
 * Раздельное удаление файлов (для Windows)
 * @param {Git} thisObj Экземпляр Git
 * @param {Array<string>} files Список файлов для удаления
 * @return {Promise} A promise.
 */
async function separateRm(thisObj, files) {
  const limitFileCount = 100; // Максимальное количество файлов за один вызов
  const fileLength = files.length;
  const loopCount = Math.ceil(fileLength / limitFileCount);

  let startIdx = 0;
  const allExecResult = [];

  for (let i = 0; i < loopCount; i++) {
    const endIdx = startIdx + limitFileCount;
    const rmFiles = files.slice(startIdx, endIdx);
  ...
  // Возвращаем результат последнего выполнения
